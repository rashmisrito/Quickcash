...
function createData(id:any,transStatus:any,createdBy:any,dueAmount:Number,invoice_number:any,invoicedate:any,due_date:String,total:String,transaction:string,status: string,action: string,currency:string,recurring:any,type:any) {
  return { id,transStatus,createdBy,dueAmount,invoice_number,invoicedate,due_date,total,transaction,status,action,currency,recurring,type };
}
...
export default function Invoices() {
  ...
  const [recFlag,setrecFlag] = React.useState<any>('');
  const [recFlagType,setrecFlagType] = React.useState<any>('');
  ...
  const handleClickOpenRecurring = (val:any,type:any) => {
    setrecFlag(val);
    setrecFlagType(type);
  ...
  const getTransactionsList = async(id:any) => {
    await axios.get(`/${url}/v1/invoice/list/${id}`, {
      headers: {
  ...
  const exportExcellistforInvoice = async() => {
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
    await axios.get(`/${url}/v1/invoice/export/${accountId?.data?.id}`, {
      headers: {
       'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
    .then(result => {
      ...
      window.open(`${import.meta.env.VITE_PUBLIC_URL}/InvoiceList.xlsx`);
    })
    .catch(error => {
      console.log("error", error);
    }) 
  ...
  rows = [
    list?.map((item:any) => (
     createData(item._id,item?.paidAmount,item.createdBy,item.dueAmount,item.invoice_number,item.invoice_date,item.due_date,item.total,item.status,item.status,item._id,item.currency,item.recurring,item?.createdBy)
    ))
  ...
  const getFilterResults = async() => {
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
     await axios.get(`/${url}/v1/invoice/list/${accountId?.data?.id}?status=${filterStatus}&recurring=${filterRecurring}`, {
      headers: {
       'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
     })
  ...
  return (
    <>
     ...
     <Box sx={{ marginTop: {xs:'-10px',md:'12px'},fontSize: '15px' }}>
      ...
      <Grid onClick={() => handleOpen()} sx={{ cursor: 'pointer' }}>
        ...
        <Grid onClick={() => invoiceSettings()} sx={{ cursor: 'pointer' }}>
          <Tooltip title="Add New Invoice" sx={{ cursor: 'pointer' }} placement="bottom-start">
            <img src={`${import.meta.env.VITE_APP_URL}/icons/add.png`} width={45} />
          </Tooltip>
        </Grid>
      </Grid>
      <Grid sx={{display: 'flex', flexDirection: 'column' , gap: '1px',borderRadius: '.5rem', color: 'black', fontWeight: '700', background: {xs: `${theme ? '#183153': 'white'}` , sm:  `${theme ? '#183153': 'white'}`} , padding: '2px'}}>      
       <div>
        <Modal
          open={open2}
            ...
            <Card>
                <CardContent>
                  <Typography sx={{ fontSize: 14 }} color="text.secondary" gutterBottom>
                    <Grid sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', color: `${theme ? 'white': 'black'}`}}>
                      <Grid>Filter Options</Grid>
                  ...
                  <Select fullWidth sx={{marginBottom: '20px',border: `${theme ? '1px solid white' : ''}`}} value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
                    ...
                    <MenuItem value="partially">Partially Paid</MenuItem>
                    <MenuItem value="paid">Paid</MenuItem>
                    <MenuItem value="unpaid">Unpaid</MenuItem>
                  </Select>
                  <label>Recurring Status :</label>
                  <Select fullWidth value={filterRecurring} sx={{ border: `${theme ? '1px solid white' : ''}` }} onChange={(e) => setFilterRecurring(e.target.value)}>
          ...
          <TableBody>
            ...
            <TableRow key={row?.invoice_date}>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} scope="row">
              <Grid sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '6px' }}>
                <Grid>
                  {row?.invoice_number}
                </Grid>
                <Grid>{row?.recurring == "yes" && <Tooltip title="Recurring Active" placement="top-start" arrow><RepeatOnIcon sx={{ color: '#8657E5' }} /></Tooltip>}</Grid>
                <Grid>
                 {
                   row?.status != "paid" && currentDate > row?.due_date && <><Tooltip title="Overdue" placement="top-start" arrow><PendingActionsIcon sx={{ color: 'red' }} /></Tooltip></>
                 }
                </Grid>
              </Grid>
             </TableCell>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} scope="row">
              <span style={{ padding: '10px', borderRadius: '10px'}}>{moment(row?.invoicedate).format("DD-MM-YYYY")}</span>
             </TableCell>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} scope="row">
             <span style={{ padding: '10px', borderRadius: '10px'}}>{moment(row?.due_date).format("DD-MM-YYYY")}</span>
             </TableCell>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} scope="row">
              {getSymbolFromCurrency(row?.currency)}{row?.total}
             </TableCell>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} align='center'  scope="row">
             {getSymbolFromCurrency(row?.currency)}{row?.transStatus}
             <br />
             </TableCell>
             <TableCell style={{ padding: '12px', fontSize: '14px', fontWeight: '400' }} scope="row">
              {(row?.status == "converted") && <><span style={{background: 'yellow', borderRadius: '12px', color: 'black' , padding: '10px'}}>Converted</span></>}
               ...
               <Grid sx={{display: 'flex', flexDirection: 'row', justifyContent: 'normal'}}>
                  <Grid>
                    ...
                    <Menu
                      ...
                      onClose={handleClose}
                    >
                      {(status == "unpaid" || status == "converted") && <MenuItem onClick={() => navigate(`/edit-invoice/${moreVal}`)}><span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}>Edit</span></MenuItem>}
                      <MenuItem onClick={() => contentCopy(`${import.meta.env.VITE_PUBLIC_URL}/invoice/${invoiceNumber}`)}><span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}>Invoice URL</span></MenuItem>
                      { status == "unpaid" && 
                       <>
                         <MenuItem onClick={() => HandleDeleteInvoice(moreVal)}>
                      ...
                      { recurring == "yes" && 
                       <>
                        <MenuItem onClick={() => HandleUpdateInvoice(moreVal,"no")}>
                          <span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}>Stop Recurring</span>
                        </MenuItem>
                       </>
                      }
                      { recurring == "no" && 
                       <>
                        <MenuItem onClick={() => handleClickOpenRecurring(moreVal,"yes")}>
                          <span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}>Start Recurring</span>
                        </MenuItem> 
                       </>
                      }
                    </Menu>
     ...
     {/* // Modal Box for Recurring Filter */}
     <Dialog
        fullWidth
        open={openRecurring}
      ...
      >
        ...
        <DialogContent>
          <Grid>
            <Grid item xs={12} sm={6}>
              ...
              <Select name="value" onChange={(e) => setRecurringCycle(e.target.value)} fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => HandleUpdateInvoice(recFlag,recFlagType)}>Save</Button>
          <Button onClick={handleCloseRecurring}>Cancel</Button>
        </DialogActions>
     </Dialog>
