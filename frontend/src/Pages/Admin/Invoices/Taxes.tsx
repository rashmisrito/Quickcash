...
export default function Taxes() {
  ...
  const handleChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await axios.patch(`/${url}/v1/admin/tax/update/${event?.target?.value}`, {
  ...
  const HandleDeleteTax = async (val:any) => {
    ...
    if(r == true){
      await axios.delete(`/${url}/v1/admin/tax/delete/${val}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admintoken')}`
        }
      })
  ...
  return (
    <>
     ...
     <Typography><ColorButton onClick={() => navigate('/admin/invoices/add-tax')} sx={{marginBottom: '12px'}}>Add Tax</ColorButton></Typography>
            ...
            <TableRow key={row?.date}>
             ...
             <TableCell component="th" scope="row">
              <IconButton
                ...
                >
                  <MenuItem><span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}><EditIcon sx={{ cursor: 'pointer' }} onClick={() => navigate(`/admin/invoices/edit-tax/${moreVal}`)}/></span></MenuItem>
