...
export default function EditQuotes() {
  ...
  const getInvNumber = async () => {
    await axios.get(`/${url}/v1/quote/${params?.id}`, {
      headers: {
    ...
    .then(result => {
      if(result.data.status == "201") {
        setInv(result.data.data[0].quote_number);
  ...
  const HandleUpdateQuote = async () => {
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
    await axios.patch(`/${url}/v1/quote/update/${params?.id}`, {
  ...
  <>
    <Toolbar sx={{ display: 'flex' , flexDirection: 'column' , padding: '12px', borderRadius: '.5rem', p: '2px', marginTop: '10px', background: {xs: `${theme ? '#183153' : ''}` , md: `${theme ? '#183153' : 'white'}`} , border: {xs: `${theme ? '1px solid white' : ''}` , md: `${theme ? '1px solid white' : 'white'}`} }}>
      <Box sx={{ marginTop: '0%', borderRadius: '.5rem' }}>
        <Typography>
          <Colorbtn sx={{cursor: 'pointer', float: 'right', marginTop: '10px'}} onClick={() => navigate('/invoices/quotes/list')}>Back</Colorbtn>
        </Typography>
            ...
            <Grid item xs={12} sm={12}>
                        ...
                        <tr>
                          <td>
                            ...
                            <tr>
                              ...
                              <td>
                                <Select sx={{border:'1px solid silver', width: '200px' , color: `${theme ? 'white': 'black'}`}} value={discountType} onChange={(e) => HandleDiscountThings(e.target.value)}>
              ...
              <Grid item xs={12} sm={12}>
                <Grid sx={{display: 'flex', flexDirection: 'row', gap: '10px' ,marginBottom: '20px', justifyContent: 'flex-end'}}>
                  <Grid><Colorbtn onClick={() => HandleUpdateQuote()}>Save & Send</Colorbtn></Grid>
                  <Grid><Colorbtn onClick={() => HandleCancelQuote()}>Cancel</Colorbtn></Grid>
                </Grid>
              </Grid>
