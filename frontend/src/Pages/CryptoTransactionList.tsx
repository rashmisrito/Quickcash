...
import { Box, Table, Grid, Paper, TableCell, TableContainer, TableHead, TableRow, Tooltip } from '@mui/material';
...
const columns = [
  ...
  { id: 'info', label: 'Coin', minWidth: 100 },
  { id: 'type', label: 'No Of Coins', minWidth: 100 },
  { id: 'amount', label: 'Type', minWidth: 100 },
  { id: 'postBalance', label: 'Amount', minWidth: 100 },
  { id: 'status', label: 'Status', minWidth: 100 }
];

function createData(ctype:any,date:any,coin:String,nofcoins:String,type:Number,amount:any,status:any) {
  return { ctype,date,coin,nofcoins,type,amount,status };
}
...
export default function CryptoTransactionList() {
  ...
  const { toPDF, targetRef } = usePDF({filename: 'cryptotransactionList.pdf'});
  ...
  const getTransactionsList = async() => {
    ...
    if(valSearch.substring(0,6) == "crypto" ) {
      await axios.get(`/${url}/v1/crypto/listbyId/${valSearch?.replace("crypto=","")}`,
      {
        headers: 
        {
  ...
  const exportExcellistforInvoice = async() => {
    ...
    await axios.get(`/${url}/v1/transaction/exportcrypto/${accountId?.data?.id}/${valSearch?.replace("crypto=","")}`, {
      headers: {
       'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
    .then(result => {
      ...
      window.open(`${import.meta.env.VITE_PUBLIC_URL}/cryptotransactions.xlsx`);
    })
    .catch(error => {
      console.log("error", error);
    });
  ...
  let rows:any = [];
  rows = [
    // date,coin,nofcoins,type,amount,status
    list?.map((item:any) => (
      createData(item?.currencyType,item.createdAt, item.coin, item.noOfCoins, item?.side, item.amount,item.status)
    ))
  ...
  return (
    <>
     <Grid sx={{display: 'flex', flexDirection: 'column' , gap: '20px',borderRadius: '.5rem', color: 'black', fontWeight: '700', background: `${theme ? '' : 'white'}` , padding: '10px 12px'}}>
      <Box sx={{ overflow: "auto" }}>
       <Box sx={{ width: "100%", display: "table", tableLayout: "fixed" }}>
        <Grid sx={{ display: 'flex', flexDirection: 'row',flexWrap: 'wrap', justifyContent: 'space-between' , gap: '10px' }}>
          <Grid sx={{ color: { xs: `${theme ? 'white': 'black'}`} }}>Transaction List</Grid>
          {
            rowsNew?.length > 0 &&
        ...
        <TableContainer component={Paper} ref={targetRef}>
          ...
          <TableBody>
            ...
            <TableRow key={row?.date}>
             ...
             <TableCell style={{ width: 160 }} component="th" scope="row">
             {getSymbolFromCurrency(row?.ctype)}{row?.amount}
             </TableCell>
             <TableCell style={{ width: 160 }} component="th" scope="row">
             {row?.status == "Success" && 
              <span 
     ...
     {/* Modal box is used for display Transaction Details by individual transaction id */}
     <Dialog
      fullWidth
      open={open}
      maxWidth="sm"
          ...
          <DialogContentText id="alert-dialog-description">
          {
            <>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <table border={1} width="100%" style={{borderCollapse: 'collapse'}} className={`${theme ? 'avatarDark' : 'avatarLight'}`}>
                    <tbody>
                      <tr>
                      ...
                      <tr>
                        ...
                        <td style={{padding: '10px'}}>
                          {listOne?.[0]?.tr_type == "UPI" && listOne?.[0]?.upi_id}
                          {
                            ...
                            ? 
                            listOne?.[0]?.receipient ?
                             `${listOne?.[0]?.senderAccountDetails?.[0]?.country}${listOne?.[0]?.senderAccountDetails?.[0]?.iban}`
                            :
                      ...
                      <tr>
                        ...
                        <td style={{padding: '10px'}}>
                        {
                          ...
                          :
                          listOne?.[0]?.receipient ?
                          `${listOne?.[0]?.recAccountDetails?.[0]?.country}${listOne?.[0]?.recAccountDetails?.[0]?.iban}`
                          :
                          listOne?.[0]?.transferAccountDetails?.[0]?.iban 
                        }
                        </td>
