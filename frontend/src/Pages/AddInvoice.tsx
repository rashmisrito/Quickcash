...
export default function AddInvoice() {
  ...
  const HandleAddInvoice = async (val:any) => {
    ...
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
    await axios.post(`/${url}/v1/invoice/add`, {
    ...
    <>
      <Toolbar sx={{ display: 'flex' , flexDirection: 'column' , borderRadius: '.5rem', p: '2px', marginTop:'10px', background:{xs: `${theme ? '#183153' : ''}`,md: `${theme ? '#183153' : 'white'}`},border: {xs: `${theme ? '1px solid white' : ''}`,md: `${theme ? '1px solid white' : 'white'}`} }}> 
        <Box sx={{ marginTop: '0%', borderRadius: '.5rem' }}>
          <Typography>
            <ColorButton sx={{cursor: 'pointer', float: 'right', marginTop: '10px'}} onClick={() => navigate('/invoice-section')}>Back</ColorButton>
          </Typography>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <RadioGroup
                row
                sx={{ padding: '2px', marginTop: '12px', borderRadius: '12px', background: `${theme ? 'white': ''}`, color: 'black' }}
                aria-labelledby="demo-row-radio-buttons-group-label"
                name="row-radio-buttons-group"
                value={memberType}
                onChange={handleChangemember}
              >
                <FormControlLabel value="member" sx={{ marginLeft: `${theme ? '12px': '0px'}` }} control={<Radio />} label="Member" />
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Select fullWidth onChange={(e) => setUserId(e.target.value)} sx={{ border: `${theme ? '1px solid silver': 'black'}` }}>
                {
                  UsersList?.map((item:any,index:number) => (
                    <MenuItem value={item?._id} key={index} sx={{ color: `${theme ? 'white': 'black'}` }}>{item?.firstName}{" "}{item?.lastName}</MenuItem>
                  ))
                }
                </Select>
              </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <label>Status</label>
              <Select fullWidth value={status} onChange={(e) => setStatus(e.target.value)} sx={{ border: `${theme ? '1px solid silver': 'black'}` }}>
                <MenuItem sx={{ color: `${theme ? 'white': 'black'}` }}>Select Status</MenuItem>
              ...
              <Select value={payment_qr_code} name="payment_qr_code" onChange={(e) => setPaymentQRCode(e.target.value)} sx={{border: '1px solid silver', borderRadius: '7px'}} fullWidth>
                {
                  QRCodeList?.map((item:any,index:number) => (
                    <MenuItem value={item?._id} key={index}>{item?.title}</MenuItem>                  
                  ))
                }
              </Select>
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              ...
              >
                <FormControlLabel value="yes" control={<Radio sx={{ marginLeft: `${theme ? '12px': '0px'}` }} />} label="Yes" />
            ...
            </Grid> 
            {
              ...
              <>
                <Grid item xs={12} sm={6}>
                  ...
                  <Select name="value" onChange={(e) => setRecurringCycle(e.target.value)} sx={{ border: `${theme ? '1px solid silver': 'black'}` }} fullWidth>
                    <MenuItem value="1" sx={{ color: `${theme ? 'white': 'black'}` }}>Day</MenuItem>
                    <MenuItem value="7" sx={{ color: `${theme ? 'white': 'black'}` }}>Weekly</MenuItem>
                    <MenuItem value="31" sx={{ color: `${theme ? 'white': 'black'}` }}>Monthly</MenuItem>
                    <MenuItem value="180" sx={{ color: `${theme ? 'white': 'black'}` }}>Half Yearly</MenuItem>
                    <MenuItem value="365" sx={{ color: `${theme ? 'white': 'black'}` }}>Yearly</MenuItem>
                  </Select>
                </Grid> 
              </>
            }  
                      ...
                      <tr>
                        ...
                        <td align='center' style={{padding: '10px'}}>
                          <TextField
                            ...
                            fullWidth
                            value={item?.price}
                            sx={{ border:'1px solid white', background: '',  borderRadius:'7px', minWidth:"200px"}}
                            inputProps={{
                            shrink: true
                            }}
                          />
                              ...
                              <td>
                                ...
                                :
                                  <TextField
                                    ...
                                    value={OverAllDiscount}
                                    onChange={(e) => HandleDiscountOverall(e.target.value || 0)}
                                    sx={{border:'1px solid silver' , borderRadius: '7px', maxWidth: '90px' , background: ''}}
                                    InputProps={{ inputProps: { min: 0, max:100 } }}
                                  />
                                }
                              </td>
                          ...
                          <td>
                            ...
                            <tr>
                              <td>
                                ...
                                >
                                  <MenuItem value="" sx={{ color: `${theme ? 'white': 'black'}` }}>Select Tax</MenuItem>
                                  {
                                    taxList?.map((items:any,indexx:any) => (
              ...
              </Grid>
              {
                ...
                <>
                  <Grid item xs={12} sm={12}>
                    <Grid container sx={{display: 'flex', marginLeft: '20px', flexDirection: 'row' , gap: '10px'}}>
