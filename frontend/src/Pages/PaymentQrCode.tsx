...
export default function PaymentQrCode() {
  ...
  const handleChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await axios.patch(`/${url}/v1/qrcode/update/${event?.target?.value}`, {
  ...
  const HandleDeleteTax = async (val:any) => {
    ...
    if(r == true){
      await axios.delete(`/${url}/v1/qrcode/delete/${val}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      })
    ...
    <>
     <Box sx={{ fontSize: '15px' }}>
      <Grid sx={{display: 'flex', flexDirection: 'column' , gap: '20px',borderRadius: '.5rem', color: 'black', fontWeight: '700', background: {xs: `${theme ? '#183153' : 'white'}` , sm:`${theme ? '' : 'white'}` },border: {xs: `${theme ? '1px solid white' : 'white'}` , sm:`${theme ? '1px solid white' : 'white'}` } , padding: '1px 1px'}}>
       <Typography><ColorButton onClick={() => navigate('/invoices/add-qr-code')}>Add QR Code</ColorButton></Typography>
        ...
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
            ...
            <TableRow key={row?.date}>
             ...
             <TableCell component="th" scope="row">
              <EditIcon sx={{ cursor: 'pointer', marginLeft: '10px' }} onClick={() => navigate(`/invoices/edit-qrcode/${row?.id}`)}/>
