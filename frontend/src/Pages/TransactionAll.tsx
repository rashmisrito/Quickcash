...
import { Box, Grid, MenuItem, Select, TextField, Tooltip, Typography } from '@mui/material';
...
export default function TransactionAll() {
  ...
  const getTransactionsList = async() => {
    ...
    await axios.get(`/${url}/v1/transaction/list/${accountId?.data?.id}`, 
    {
      headers: 
      {
  ...
  const exportExcellistforInvoice = async() => {
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
    await axios.get(`/${url}/v1/transaction/exportstatement/${accountId?.data?.id}?status=${localStorage.getItem('status') || ''}&account=${localStorage.getItem('accountFilter') || ''}&transType=${localStorage.getItem('transactionType') || ''}&startDate=${localStorage.getItem('startDate') || ''}&endDate=${localStorage.getItem('endDate') || ''}`, {
      headers: {
       'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
  ...
  const FilterTransaction = ({setList}:any) => {
    ...
    const getTransactionsFilterList = async() => {
      setPage(0);
      const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
      await axios.get(`/${url}/v1/transaction/list/${accountId?.data?.id}?transType=${transactionType}&status=${status}&account=${accountIdq}&startDate=${startDate}&endDate=${endDate}`, 
      {
        headers: 
        {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        <Grid sx={{display: 'flex', flexDirection: 'row', justifyContent: 'normal', flexWrap: 'wrap', gap: '12px'}}>
            ...
            <Grid sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between',flexWrap: 'wrap',  gap: '12px'}}>
              <Grid sx={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '12px'}}>
                 ...
                 <Grid className={`${theme ? 'clLight': ''}`}>
                  <TextField type="date" sx={{  color: `${theme ? 'white': 'black'}`, border: `${theme ? '1px solid white': ''}` }} value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                 </Grid>
              </Grid>
              <Grid sx={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '12px'}}>
                 <Grid>
                 ...
                 <Grid className={`${theme ? 'clLight': ''}`}>
                   <TextField type="date" sx={{ border: `${theme ? '1px solid white': ''}`, color: `${theme ? 'white' : 'black'}` }} value={endDate} onChange={(e) => setEndDate(e.target.value)}  />
                 </Grid>
              </Grid>
            </Grid>
          </Grid>
          ...
          <Grid>
            <Grid sx={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '12px'}}>
              ...
              <Grid sx={{ border: `${theme ? '1px solid white': ''}` }}>
                <Select value={transactionType} onChange={(e) => setTransactionType(e.target.value)} fullWidth sx={{ minWidth: '150px', color: `${theme ? 'white': 'black'}`}}>
          ...
          <Grid>
            <Grid sx={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '12px'}}>
              ...
              <Grid sx={{ border: `${theme ? '1px solid white': ''}` }}>
                <Select value={status} onChange={(e) => setStatus(e.target.value)} fullWidth sx={{minWidth: '150px', color: `${theme ? 'white': 'black'}`}}>
                  <MenuItem value="pending" sx={{ color: `${theme ? 'white': 'black'}` }}>Pending</MenuItem>
                  <MenuItem value="succ" sx={{ color: `${theme ? 'white': 'black'}` }}>Complete</MenuItem>
                  <MenuItem value="denied" sx={{ color: `${theme ? 'white': 'black'}` }}>Denied</MenuItem>
                </Select>
              </Grid>
            </Grid> 
          </Grid>
  ...
  return (
    <>
     <Grid sx={{display: 'flex', flexDirection: 'column' , marginTop: { xs: '-10px' , md: '3px' }, gap: '20px',borderRadius: '.5rem', color: `${theme ? 'white' : 'black'}`, fontWeight: '700', border: `${theme ? '2px solid lightblue': '2px solid transparent'}` , padding: {xs:'10px', md:'10px 12px'}}}>
        <Grid sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' , gap: '10px' }}>
          ...
          <Breadcrumbs aria-label="breadcrumb" sx={{ color: {xs: `${theme ? 'white': 'black'}` , sm: `${theme ? 'white': 'black'}`} }}>
            ...
            <Typography component={'span'} sx={{ color: {xs: `${theme ? 'white': 'black'}` , sm: `${theme ? 'white': 'black'}`} }}>Transaction List</Typography>
          </Breadcrumbs>
          </Grid>
          <Grid sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' , gap: '10px' }}>
          {
      ...
      <Grid sx={{display: 'flex', flexDirection: 'column' , gap: '20px', borderRadius: '.5rem', color: 'black', fontWeight: '700', padding: {xs:'10px', md:'10px 12px'}}}>
        {/* @ts-ignore */}
       ...
       {/* Modal box is used to display Individual Transactions Details by their Id */}
      <Dialog
        fullWidth
        open={open}
        maxWidth="sm"
          ...
          <DialogContentText id="alert-dialog-description">
          {
            <>
              <Grid container spacing={2} className={`${theme ? 'avatarDark' : 'avatarLight'}`}>
                <Grid item xs={12}>
                  <table border={1} width="100%" style={{borderCollapse: 'collapse', border: `${theme ? '1px solid lightblue': '1px solid silver'}`}}>
                    <tbody>
                      <tr>
