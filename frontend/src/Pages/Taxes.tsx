...
function createData(id:any,date:any, name:String , value:String , IsDefault:string) {
  return { id,date,name,value,IsDefault };
}
...
export default function Taxes() {
  ...
  const handleChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await axios.patch(`/${url}/v1/tax/update/${event?.target?.value}`, {
  ...
  const HandleDeleteTax = async (val:any) => {
    ...
    if(r == true){
      await axios.delete(`/${url}/v1/tax/delete/${val}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      })
      .then(result => {
        if(result.data.status == "201") {
          const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
          getTransactionsList(accountId.data.id);
          alertnotify("Selected tax deltails has been deleted Successfully", "success");
        }
      })
      .catch(error => {
        console.log("error", error);
  ...
  return (
    <>
     <Box sx={{ fontSize: '15px' }}>
      <Grid sx={{display: 'flex', flexDirection: 'column' , gap: '20px',borderRadius: '.5rem', color: 'black', fontWeight: '700', background: {xs: `${theme ? '#183153' : 'white'}` , sm:`${theme ? '' : 'white'}` },border: {xs: `${theme ? '1px solid white' : 'white'}` , sm:`${theme ? '1px solid white' : 'white'}` }, padding: '1px 1px'}}>
       <Typography><ColorButton onClick={() => navigate('/invoices/add-tax')}>Add Tax</ColorButton></Typography>
            ...
            <TableRow key={row?.date}>
             ...
             <TableCell component="th" scope="row">
              <IconButton
                ...
                aria-controls={open2 ? `long-menu${row?.id}` : undefined}
              ...
              >
                ...
                >
                  <MenuItem><span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}><EditIcon sx={{ cursor: 'pointer' }} onClick={() => navigate(`/invoices/edit-tax/${moreVal}`)}/></span></MenuItem>
                  <MenuItem><span className={`${theme ? 'avatarDarkSecondaryExtra' : 'avatarLight'}`}><DeleteIcon sx={{ cursor: 'pointer' }} onClick={() => HandleDeleteTax(row?.id)} /></span></MenuItem>
                </Menu>
             </TableCell>
            </TableRow>
          ))}
