...
import { Box, Table, Grid, Paper, TableCell, TableContainer, TableHead, TableRow, Tooltip, Typography } from '@mui/material';
...
export default function TransactionList({...props}) {
  ...
  const getTransactionsList = async() => {
    ...
    if(valSearch == "all") {
      ...
      const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
      await axios.get(`/${url}/v1/crypto/listbyId/DOGE_TEST`,
      {
        headers: 
        {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
      await axios.post(`/${url}/v1/transaction/account`, {
        "user_id": accountId?.data?.id,
        "account": props?.currency?._id,
  ...
  const exportExcellistforInvoice = async() => {
    const accountId = jwtDecode<JwtPayload>(localStorage.getItem('token') as string);
    await axios.get(`/${url}/v1/transaction/export/${accountId?.data?.id}`, {
      headers: {
       'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
    .then(result => {
      ...
      window.open(`${import.meta.env.VITE_PUBLIC_URL}/transactions.xlsx`);
    })
    .catch(error => {
      console.log("error", error);
    });
          ...
          <TableBody>
            ...
            <TableRow key={row?.date}>
             ...
             <Tooltip title="View transaction details" placement="top" arrow>
               <Typography sx={{ color: {xs: `${theme ? 'white': 'black'}`} }}>{row?.trx}</Typography>
              </Tooltip>
             </TableCell>
